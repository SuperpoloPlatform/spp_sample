...
try {
	...
	// 改变环境光的亮度。
	function changeAmbient(value) {
		// chenyang:这两行是伪代码，想好事呢。。。 ticket:1031
		// var sec = SceneTree.Get("world/sectors[0]");
		// sec.ambient = sec.ambient.Add2([value, value, value]);
		...
		sector.ambient = sector.ambient.Add2([value, value, value]);
	}
	
	var ent = Entities.CreateEntity();
	ent.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){
		// 改变环境光
		if(msgid.indexOf('lightup1')!=-1) {
			changeAmbient(0.1);
		}
		if(msgid.indexOf('lightdown1')!=-1) {
			changeAmbient(-0.1);
		}
	...
	// 改变环境光的快捷键。
	prop_input.PerformAction('Bind',['trigger', 'PadPlus'], ['command','lightup']);
	prop_input.PerformAction('Bind',['trigger', 'PadMinus'], ['command','lightdown']);
