...
try {
	...
	prop_input.PerformAction('Bind',['trigger','MouseAxis0'],['command','mousemove']);
	 ...
	 C3D.engine.SubscribeFrame(function(rv){ 
		...
		prop_bb.SetProperty("text_offset",[20000,20000]);//调整位置，设置偏移量
		prop_bb.SetProperty("text","billboard"); //不支持中文，显示是乱码
		prop_bb.SetProperty("visible",true);  //设置是否可见
	    prop_bb.SetProperty("width",61440); //调整图片的宽度
	    prop_bb.SetProperty("height",34800); //调整图片的高度
	    prop_bb.SetProperty("x",x*307200); //所在屏幕位置的x轴坐标
        prop_bb.SetProperty("y",y*307200);  //所在屏幕位置的y轴坐标
		//prop_bb.SetProperty("text_default_font_size","12");  //字体大小
       // prop_bb.SetProperty("text_default_font","/fonts/simhei.ttf"); //设置字体
	 });
	 
	 //鼠标点击billboard面板响应
	bb.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){        
	...
	////////////////////////////////////////////////////////
	//prop_bb.PerformAction("DrawMesh", ["materialname", "bb"], ["factory", "genCube"]);
