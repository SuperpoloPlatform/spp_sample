...
try {
	...
	ent.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){
		if(msgid.indexOf('GammaUp1')!=-1) {
			C3D.g2d.gamma += 0.1;
		}
		if(msgid.indexOf('GammaDown1')!=-1) {
			C3D.g2d.gamma += -0.1;
		}
	...
	// 可以使用组合键
	prop_input.SetProperty('cooked',true);
	...
	prop_input.PerformAction('Bind',['trigger','PadPlus'],['command','GammaUp']);
	prop_input.PerformAction('Bind',['trigger','PadMinus'],['command','GammaDown']);
