...
try {
	...
	ent.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){
		...
		if(msgid.indexOf('changematerial0')!=-1) {
			var materialbox = C3D.engine.FindMaterial("Box01");
			var materialsky = C3D.engine.FindMaterial("sky");
			prop_pm.PerformAction('MoveMesh', ['position', [0, 100000, 30]])
			if(C3D.engine.FindMeshObject("sky").meshObject.matWrap == materialsky)
			{
				C3D.engine.FindMeshObject("sky").meshObject.matWrap = materialbox;
			} else {
				C3D.engine.FindMeshObject("sky").meshObject.matWrap = materialsky;
			}
		}
	}
	ent.name = 'effect';
	...
	prop_input.PerformAction('Bind',['trigger','k'],['command','changematerial']);
