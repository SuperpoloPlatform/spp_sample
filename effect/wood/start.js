...
try {
	...
	ent.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){
		...
		if(msgid.indexOf('test1')!=-1) {
			//alert(prop_steer);
			
			
			var base = Registry.Create("cel.celgraph");
			//在iCelGraph被wrap之后返回graph对象。
			var celgraph = base.QueryInterface("iCelGraph");
			alert(base);
			alert(celgraph);
			...
			celgraph.AddEdge(gn0, gn3, true);

			celgraph.AddEdge(gn1, gn0 , true);
			celgraph.AddEdge(gn2, gn1 , true);
			celgraph.AddEdge(gn3, gn0 , true);
			
			//alert(prop_pathfinder);
			prop_steer.CheckArrivalOn(0.1);
			//prop_pathfinder.PerformAction('SetGraph',[]);
			//prop_pathfinder.PerformAction('Seek',[]);
			prop_pathfinder.SetGraph(celgraph);
			prop_pathfinder.Seek(setctor, [0,0,0]);
		}
	...
	prop_pathfinder = Entities.CreatePropertyClass(ent,'pcpathfinder');
	
	prop_steer = Entities.CreatePropertyClass(ent,'pcsteer');
	...
	prop_cam.SetProperty("distance",0.5);
	...
	//alert(prop_pm.PerformAction('AttachSocketMesh',['socket','jian2.jpg'],['object','person2']));
	...
	prop_ment.PerformAction('InitCD',['offset',[0, -0.5, 0]],['body',[0.5,0.5,0.5]],['legs',[0.5,0.9,0.5]]);
