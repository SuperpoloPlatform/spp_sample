...
try {
	...
	// 创建monster
	var monster = Entities.CreateEntity();
	monster.name = 'monster';
	
	// monster绑定到一个mesh上。
	m_prop_pm = Entities.CreatePropertyClass(monster, 'pcmesh');
	m_prop_pm.PerformAction('SetMesh', ['name','monster']);
	
	// 设定player走进monster所在的区域，monster的触发器被相应。
	m_prop_trig = Entities.CreatePropertyClass(monster, 'pclogic.trigger');
	m_prop_trig.SetProperty("monitor", 'player');
	m_prop_trig.PerformAction("SetupTriggerSphere", ['sector', 'Scene'], ['position', [0,0,-5]], ['radius', 1]);
	
	// 当走进区域的时候显示弹出窗口。
	// 当离开区域的时候显示弹出窗口。
	monster.behaviour = function(msgid,pc,p1,p2,p3,p4,p5){
		if(msgid == "pctrigger_entityenters") alert("Player进入Monster的（攻击）区域！！");
		if(msgid == "pctrigger_entityleaves") alert("Player离开Monster的（攻击）区域。");
	}
